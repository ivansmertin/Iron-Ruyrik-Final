...
try{slots[3].booked=7;slots[3].available=1;await go('/booking/s3','last7-before');await p.clock.pauseAt(new Date('2026-09-12T08:01:00+03:00'));await p.getByRole('button',{name:'Подтвердить запись'}).evaluate(e=>{e.click();});await new Promise(r=>setTimeout(r,150));await p.clock.runFor(100);results.push({at100:await p.locator('body').innerText()});await p.screenshot({path:dir+'/last7-100.png'});await p.clock.runFor(249);results.push({at349:await p.locator('body').innerText()});await p.clock.runFor(1);await p.clock.resume();await snap('last7-after');}finally{writeFileSync(dir+'/last7-results.json',JSON.stringify(results,null,2));await ctx.close();await browser.close();}
