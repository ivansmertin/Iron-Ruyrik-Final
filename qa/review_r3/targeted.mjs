...
try{
await go('/progress','flow-manual-before');await p.getByRole('button',{name:'Записать новый замер'}).click();await p.getByLabel('Вес (кг)',{exact:true}).fill('77.5');await p.getByRole('button',{name:'Сохранить',exact:true}).click();await p.waitForTimeout(800);await snap('flow-manual-after');
await go('/','flow-home');await p.locator('a[href="/schedule"]').first().click();await p.locator('a[href*="/booking/s3"]').click();await p.getByRole('button',{name:'Подтвердить запись'}).click();await p.waitForTimeout(800);await snap('flow-booked');await p.getByRole('button',{name:'Отменить запись',exact:true}).click();await p.getByRole('dialog').getByRole('button',{name:'Отменить запись',exact:true}).click();await snap('flow-cancelled');await p.getByRole('link',{name:'Профиль',exact:true}).click();await snap('flow-profile');await p.getByRole('button',{name:'Показать всю историю посещений'}).click();results.push({historyRows:await p.locator('[role="dialog"] .history-row').count(),rootInert:await p.locator('#root').evaluate(e=>e.inert)});await p.keyboard.press('Escape');results.push({focusAfterEscape:await p.evaluate(()=>document.activeElement?.getAttribute('aria-label')),rootInert:await p.locator('#root').evaluate(e=>e.inert)});
await go('/trainers','flow-trainers');await p.getByRole('link',{name:'Профиль тренера Дима'}).click();await p.getByRole('link',{name:'Выбрать время',exact:true}).click();await p.locator('a[href*="/booking/s3"]').click();await snap('flow-trainer-booking');
await go('/schedule','back-source');await p.locator('a[href*="/booking/s3"]').click();await p.waitForTimeout(500);await p.goBack({waitUntil:'domcontentloaded'});await snap('history-back');
await p.emulateMedia({reducedMotion:'reduce'});await go('/trainers','reduced-before');await p.getByRole('link',{name:'Профиль тренера Дима'}).click();await snap('reduced-after');
await p.emulateMedia({reducedMotion:'no-preference'});await go('/schedule','no-vt-before');await p.evaluate(()=>document.startViewTransition=undefined);await p.locator('a[href*="/booking/s0"]').click();await snap('no-vt-after');
await go('/profile','contrast-profile');results.push({disabledStyles:await p.locator('.settings-row.is-disabled small').evaluateAll(es=>es.map(e=>({text:e.textContent,color:getComputedStyle(e).color,opacity:getComputedStyle(e.closest('.settings-row')).opacity,bg:getComputedStyle(document.body).backgroundColor})))});
}catch(e){results.push({harnessError:String(e)});}finally{writeFileSync(dir+'/targeted-results.json',JSON.stringify(results,null,2));await ctx.close();await browser.close();}
