...
try{
trainers.length=0;await go('/booking/s0?trainer=dima','empty-roster');results.push({radios:await p.getByRole('radio').evaluateAll(es=>es.map(e=>({checked:e.checked,value:e.value})))});await p.getByRole('button',{name:'Подтвердить запись'}).click();await p.waitForTimeout(700);await snap('empty-roster-submitted');
}catch(e){results.push({harnessError:String(e)});}finally{writeFileSync(dir+'/trainer-risk-results.json',JSON.stringify(results,null,2));await ctx.close();await browser.close();}
